...
import agentRoutes from './routes/agentRoutes';
import chatRoutes from './routes/chatRoutes';
import dashboardRoutes from './routes/dashboadRoutes'; // Note: Fixed filename assumption (was 'dashboadRoutes')
...
// Health check route (placed early for accessibility)
...
app.get('/', (req: Request, res: Response) => {
    res.json({ message: 'ATW HQ API is running!' });
});
...
const startServer = async () => {
    try {
        ...
        // Listen on the HTTP server instance for proper port exposure
...
// Export for testing/modularity (after setup)
export { io, app, server };
